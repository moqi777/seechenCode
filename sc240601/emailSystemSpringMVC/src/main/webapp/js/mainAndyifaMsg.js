...
let readEmail = (b)=>{
    ...
    //跳转到控制层进行查询该邮件
    //并对邮件的已读性进行判断
    location.href="/email/readEmail?email_id="+email_id;
}
//点击全选按钮事件
...
let searchinp = ()=>{
    ...
    location.href="/email/selectEmail?fromOrTo=0&index=1&value="+value;
}
//删除勾选事件
let deleteCheckbox = (currentIndex,mainOrYifaMsg)=>{
    ...
    if (b){
        ...
        if (i>0){
            ...
            myForm.action = "/email/deleteCheckbox?currentIndex="+currentIndex+"&mainOrYifaMsg="+mainOrYifaMsg;
            myForm.submit();
        }
    }
}
...
let yesRead = (currentIndex)=>{
    ...
    checkbox.forEach(value => {
        if (value.checked==true){
            let myForm = document.getElementById("myform");
            myForm.action = "/email/yesRead?currentIndex="+currentIndex;
            myForm.submit();
        }
    })
}