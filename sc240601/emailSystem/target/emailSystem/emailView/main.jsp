	...
	<p style="text-align:right;">
		...
		<c:if test="${page.currentIndex != 1}">
			...
			<c:if test="${not empty value}">
				<a href="/email?type=selectEmail&fromOrTo=0&index=${page.currentIndex - 1}&value=${value}">上一页</a>
			</c:if>
		</c:if>
		<c:if test="${page.currentIndex == 1}">
		...
		<c:if test="${page.currentIndex != page.totalPages}">
			...
			<c:if test="${not empty value}">
				<a href="/email?type=selectEmail&fromOrTo=0&index=${page.currentIndex + 1}&value=${value}">下一页</a>
			</c:if>
		</c:if>
		<c:if test="${page.currentIndex == page.totalPages}">
